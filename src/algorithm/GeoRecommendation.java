...
public class GeoRecommendation {
	public List<Item> recommendItems(String userId, double lat, double lon){
		...
		DBConnection conn = DBConnctionFactory.getConnection();
		try {
			// step 1 get all favorite items
			Set<String> favoriteItems = conn.getFavoriteItemIds(userId);
			
			// step 2 get all categories of favorite items, sorting by counts
...
//					}
			...
			// step 3 do search based on category, filter out favorite items and sort by distance
