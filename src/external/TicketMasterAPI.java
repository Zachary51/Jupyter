...
public class TicketMasterAPI {
	...
	private static final String API_KEY = "XQHdqcNXpwB6bOutnnuvdEjCgkFTt6i2";
...
//		// Make URL
	...
	private String getImageUrl(JSONObject event) throws JSONException{
		if(!event.isNull(IMAGES)) {
			...
			for(int i = 0; i < images.length(); i++) {
				...
				String imageUrl = sBuilder.toString();
	...
	private Set<String> getCategories(JSONObject event) throws JSONException{
		...
		if(!event.isNull(CLASSIFICATIONS)) {
			...
			for(int i = 0; i < classification.length(); i++) {
				JSONObject genre = classification.getJSONObject(i);
				if(!genre.isNull(SEGMENT)) {
					JSONObject segment = genre.getJSONObject(SEGMENT);
	...
	private void queryAPI(double lat, double lon) {  
		List<Item> itemList = search(lat, lon, null); // 不能用enhanced for loop 因为 JSONArray没有iterable
			try {
